...
final class NoteView: BaseView {
    ...
    override func configure() {
        ...
        sentenceView.onMemoTextViewFocused = { [weak self] in
            self?.currentFocusedInput = .memoTextView
        }
        emotionView.onEmotionSelected = { [weak self] emotion in
            self?.eventPublisher.send(.didSelectEmotion(emotion))
        }
...
private extension NoteView {
    ...
    private func adjustForKeyboard(height: CGFloat) {
        ...
        contentStackView.arrangedSubviews.enumerated().forEach { index, view in
            ...
            if index == pageControl.currentPage, height > 0 {
                ...
                case .memoTextView:
                    if index == 0 { scrollToMemoTextView(in: scrollView) }
                case .none:
                    break
                }
            }
    ...
    private func scrollToMemoTextView(in scrollView: UIScrollView) {
        guard let sentenceView = pageViews.first as? SentenceRegistrationView else { return }

        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            let memoTextViewFrame = sentenceView.memoTextViewFrame
            let memoTextViewGlobalFrame = sentenceView.convert(memoTextViewFrame, to: scrollView)

            let targetY = max(0, memoTextViewGlobalFrame.minY - 20)
...
extension NoteView {
    ...
    func setDetailEmotions(_ detailEmotions: [DetailEmotion]) {
        emotionView.setDetailEmotions(detailEmotions)
    }

    /// 현재 선택된 세부감정 반환
    func getSelectedDetailEmotions() -> [DetailEmotion] {
        return emotionView.getSelectedDetailEmotions()
    }

    /// 감정 로딩 상태 설정
    func setLoadingEmotions(_ isLoading: Bool) {
        emotionView.setLoadingEmotions(isLoading)
    }
}
